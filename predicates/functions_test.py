...
def test_replace_relations_with_functions_in_model(debug=False):
    ...
    if debug:
        print(
            "Replacing relations F, GG with functions in model", model, "..."
        )
    new_model = replace_relations_with_functions_in_model(
        model, frozenset({"f", "gG"})
    )
    if debug:
        print("... got", new_model)
    assert new_model.universe == model.universe
    ...
    assert new_model.function_interpretations["f"] == {
        ("a",): "b",
        ("b",): "b",
    }
...
def test_replace_functions_with_relations_in_formula(debug=False):
    for s, valid_model, invalid_model in [
        ...
        if debug:
            print(
                "Replacing functions with relations in formula", formula, "..."
            )
...
def test_replace_functions_with_relations_in_formulas(debug=False):
    ...
    new_formulas = replace_functions_with_relations_in_formulas(
        frozenset({formula})
    )
    if debug:
...
def test_replace_equality_with_SAME_in_formulas(debug=False):
    ...
    if debug:
        print("... got", new_formulas)
    assert (
        Formula.parse("Ax[Ay[Az[((S(x,y)&S(x,z))->SAME(y,z))]]]")
        in new_formulas
    )
...
def test_add_SAME_as_equality_in_model(debug=False):
    ...
    assert (
        new_model.relation_interpretations["Q"]
