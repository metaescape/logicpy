...
def test_uniquely_rename_quantified_variables(debug=False):
    ...
    for formula in [
        ...
        if debug:
            print(
                "Testing _uniquely_rename_quantified_variables on",
                formula,
                "...",
            )
...
def test_pull_out_quantifications_across_negation(debug=False):
    ...
    for formula, expected in [
        ...
        if debug:
            print(
                "Testing _pull_out_quantifications_across_negation on",
                formula,
                "...",
            )
...
def test_pull_out_quantifications_from_left_across_binary_operator(
    debug=False,
):
    from predicates.prenex import (
...
def test_pull_out_quantifications_from_right_across_binary_operator(
    debug=False,
):
    from predicates.prenex import (
...
def test_pull_out_quantifications_across_binary_operator(debug=False):
    ...
    for formula, expected in [
        ...
        result, proof = _pull_out_quantifications_across_binary_operator(
            formula
        )
        if debug:
            print("... got", result)
...
def test_to_prenex_normal_form_from_uniquely_named_variables(debug=False):
    ...
    for formula, pnf in [
        ...
        result, proof = _to_prenex_normal_form_from_uniquely_named_variables(
            formula
        )
        if debug:
            print("... got", result)
