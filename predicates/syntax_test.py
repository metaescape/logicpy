...
def test_term_substitute(debug=False):
    ...
    # add a test for no substitution but with forbidden variables
    substitution_map = {
        "_": Term.parse("x"),
    }
    result = Term.parse("h(z)").substitute(substitution_map, {"x"})
    if debug:
        print(
            "Substituting",
            substitution_map,
    ...
    assert str(result) == "h(z)", f"Expected {h(z)}, but got {result}"

    # Test illegal substitutions
    term = Term.parse("h(c,f(x))")
    for substitution_map, forbidden_variables, variable_name in [
        (
            {"c": Term.parse("g(f(x))"), "x": Term.parse("f(c,y)")},
            {"z", "y"},
            "y",
        ),
        (
            {"c": Term.parse("g(f(y))"), "x": Term.parse("f(c,x)")},
            {"y", "z"},
            "y",
        ),
    ]:
...
def test_formula_repr(debug=False):
    ...
    formula = Formula(
        ...
        Formula(
            "A",
            "x",
            Formula("=", [Term("plus", [Term("x"), Term("y")]), Term("0")]),
        ),
...
def test_formula_substitute(debug=False):
    for s, substitution, expected in [
        ...
        formula = Formula.parse(s)
        substitution_map = {
            v: Term.parse(substitution[v]) for v in substitution.keys()
        }
    ...
    for s, substitution, forbidden_variables, variable_name in [
        ...
        try:
            result = str(
                formula.substitute(substitution_map, forbidden_variables)
            )
            assert False, "Expected exception"
        except ForbiddenVariableError as e:
            if debug:
