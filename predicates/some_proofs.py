...
def prove_homework(print_as_proof_forms: bool = False) -> Proof:
    ...
    step6 = prover.add_tautological_implication('((Homework(x)&Reading(x))->(~Fun(x)&Reading(x)))', {step5})
    ...
    step10 = prover.add_tautological_implication('((Homework(x)&Reading(x))->Ex[(Reading(x)&~Fun(x))])', {step7, step8})

    step11 = prover.add_existential_derivation('Ex[(Reading(x)&~Fun(x))]', step9, step10)
    return prover.qed()
...
def prove_group_unique_zero(print_as_proof_forms: bool = False) -> Proof:
    ...
    step1 = prover.add_substituted_equality(
        'plus(minus(x),plus(a,c))=plus(minus(x),a)',
        ac_a, 'plus(minus(x),_)')  
    ...
    step6 = prover.add_chained_equality(
        '0=c',
        [flipped_step3, flipped_step1, step2, step4, step5])
