...
class Model(Generic[T]):
    ...
    def evaluate_term(self, term: Term,
        ...
        if is_variable(term.root):
            ...
            return self.function_interpretations[term.root][tuple(self.evaluate_term(x, assignment) for x in term.arguments)] 
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        if is_equality(formula.root):
            left, right = formula.arguments
            return self.evaluate_term(left, assignment) == self.evaluate_term(right, assignment)
            ...
            if formula.root == "A":
                return all(self.evaluate_formula(formula.statement,  {**assignment, formula.variable : x}) for x in self.universe)
            elif formula.root == "E":
                return any(self.evaluate_formula(formula.statement,  {**assignment, formula.variable : x}) for x in self.universe)
