...
def test_specialization_map(debug=False):
    for t in rules:
        g = InferenceRule(
            [Formula.parse(f) for f in t[2]], Formula.parse(t[0])
        )
        s = InferenceRule(
            [Formula.parse(f) for f in t[3]], Formula.parse(t[1])
        )
...
def test_is_valid(debug=False):
    ...
    proof = Proof(
        InferenceRule(
            [Formula.parse("p"), Formula.parse("(x|y)")],
            Formula.parse("(y|x)"),
        ),
...
def test_prove_specialization(debug=False):
    ...
    for instance_infix in [
        ...
        instance = InferenceRule(
            [Formula.parse(instance_infix[0])],
...
def test_inline_proof_once(debug=False):
    ...
    proof = Proof(
        InferenceRule(
            [Formula.parse("((p|q)|r)")], Formula.parse("((r|p)|q)")
        ),
        {rule0, rule1, rule2},
        [
...
def first_use_of_rule(proof, rule):
    ...
    i = 0
    for i in range(len(proof.lines)):
        if (not proof.lines[i].is_assumption()) and proof.lines[
            i
        ].rule == rule:
            return i
    assert False
